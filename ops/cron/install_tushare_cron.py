#!/usr/bin/env python3
"""Install or refresh the managed ADM-Cube TuShare cron block."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Append or refresh the ADM-Cube TuShare cron block without replacing other crontab entries.")
...
def validate_managed_markers(text: str, *, source: str, required: bool = False) -> None:
...
def replace_managed_block(current: str, managed: str) -> str:
    ...
    validate_managed_markers(managed, source="generated block", required=True)
    kept: list[str] = []
    skipping = False
...
def verify_installed_crontab(expected: str, installed: str) -> None:
    validate_managed_markers(installed, source="installed crontab", required=True)
...
def main() -> int:
    ...
    print("installed ADM-Cube TuShare cron block")
    return 0
