...
- Test 只形成事后紧凑诊断，不能用于当前 Fold 选择、调参或回滚；Held-out 永远不可见。没有可接受更新时由 Pipeline 保留父制品，不要为了产生改动而改动。
...
- 只做消除接口疑问所需的轻量探查，随后立即写出最小可执行策略，完成静态验证与修改检查，并尽早调用 `daily_backtest` 建立正式基线。不要在 `workspace/` 自建全历史模拟器代替正式 Validation。最小垂直链路是：读已确认特征 → 仅在真实候选与执行条件成立时生成合法 JSON 订单 → 正式回测 → 检查成交/拒单与权益。没有真实候选时返回 `[]` 是正确策略结果。
...
- 结果足够或继续搜索的边际价值低于预算时，按提交合同收尾并调用 `finish_fold`。
...
先保障完整 Validation、执行可行性和回撤硬约束；版本表现接近时保留更小、更简单的信号与交易逻辑。继续搜索边际收益不值得预算时主动 finish_fold。\
"""

EXPLORATION_PHASE_PROMPT = """\
当前处于探索期：围绕清晰、可证伪的机制假设自由探索；有解释的失败也为后续 Fold 提供信息。不要无假设随机改动。\
"""

CONVERGENCE_PHASE_PROMPT = """\
当前处于收敛期：优先复验父产物或已完成 Step，只在证据明确时做最小修改，不引入大规模新框架。\
"""
...
def build_system_prompt(
    ...
    *,
    ...
    if mode in {"meta", "meta_learning"}:
        sections = [META_SYSTEM_PROMPT, RUNTIME_SYSTEM_PROMPT]
    ...
    return "\n\n".join(
        (PROTOCOL_INSTRUCTION, FOLD_DYNAMIC_CONTEXT_HEADER, *context_parts)
    )
