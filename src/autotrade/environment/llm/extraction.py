...
JSON-mode content or a single JSON object in plain text is accepted. One
```json fence may be removed. Closed <think> blocks are stripped and preserved
for logging; an unclosed <think> or multiple JSON objects is a failure.
Thinking text never supplies formal fields.
"""
...
def extract_json_object(content: str) -> ExtractedJSON:
    """Extract exactly one JSON object from a provider response.

    The content must be one JSON object, optionally wrapped in a single json
    code fence and/or preceded by a closed think block.
    """
    text = content.strip()
    stripped_think = ""
