...
def maybe_rebuild_sandbox_image(
    ...
    *,
    ...
    command = [
        base_spec.docker_executable, "build", "--tag", image_tag, "--file", str(dockerfile), str(build_dir)
    ]
...
def _render_sandbox_extension_dockerfile(base_image: str, request: dict[str, object]) -> str:
    ...
    apt_packages = [shlex.quote(str(item)) for item in request.get("apt_packages", [])]
    if apt_packages:
        lines.append(
    ...
    python_specs = [str(item) for item in request.get("python_packages", [])]
    ...
    npm_packages = [shlex.quote(str(item)) for item in request.get("npm_packages", [])]
    if npm_packages:
