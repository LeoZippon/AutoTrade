...
def agent_visible_ref(value: object, *, prefix: str) -> str:
    ...
    This is the ONLY SHA256 use in the repository, and it is deliberate: it is a
    one-way pseudonymiser, not an integrity check. Nothing is ever compared
    against a stored digest here, so a sweep for hash *verification* must leave
    it in place — without it there is no stable Agent-visible opaque ref and the
    whole anti-leakage projection has nothing to project through."""
